...
0  1       -1  0 \endcode
    ...
    struct Source {
      ...
      void sub(const Source &other, Source *res) {
	if (other.isSet && isSet) {
	  res->fromX = fromX - other.fromX;
	  res->fromY = fromY - other.fromY;
	}
      }
      void add(const Source &other, Source *res) {
	if (other.isSet && isSet) {
	  res->fromX = fromX + other.fromX;
	  res->fromY = fromY + other.fromY;
	}
...
private:
    ...
    struct SourceSum {
        ...
        void norm(Source *source) {
	  if (count == 0)
	    return;
	  else
	    {
	      source->fromX = x/count;
	      source->fromY = y/count;
	    }
        }
